...
export default function BodyMainSidebar() {
  const [handleDropDownButton, sethandleDropDown] = useState<boolean[]>([
    false,
    false,
  ]);
  ...
  return (
    <ul
      className={`w-[90%]  flex justify-center gap-1.5 flex-col 
          transform transition-all ease-in-out rounded-[.5em] overflow-hidden
          items-center ${
            handleDropDownButton.some((isOpen) => isOpen)
              ? " m-2.5 h-[38em] delay-150  duration-750"
    ...
    >
      ...
      >
        ...
        >
          ...
          <p
            className={`w-[80%] h-full justify-center
                  items-center ${bebasNeue.className} ${isOpen ? "flex" : "hidden"}`}
          >
            Notifications
          </p>
        </button>
      ...
      {/* Dropdown 1 */}
      <li
        className={`w-full  text-[#fff] flex items-center flex-col 
      ...
      >
        <button
          className=" w-full h-[3em] bg-[#151515] flex justify-center px-0.5 items-center z-20
                flex-row active:bg-[#fff] active:text-[#151515]"
          onClick={() => handleDropDown(0)}
        >
          <ShoppingBagIcon className="size-7" />
          <p
            className={`w-[80%] h-full  flex justify-end gap-9  
                    items-center ${bebasNeue.className}  ${isOpen ? "flex" : "hidden"}`}
          >
            Store
            <ChevronDownIcon
        ...
        <ul
          className={` w-[90%] h-[15em] flex flex-col bg-[#151515]
        ...
        >
          {dropdownsLists[0].map((value, index) => (
            <li
              key={index}
              className={`w-full h-[2.5em] border-0
            ...
            >
              ...
              >
                <div
                  className="w-[2em] h-[2em] bg-[#fff] rounded-[.5em] flex justify-center items-center
                            text-[#151515]  "
                >
                  <FontAwesomeIcon
                    icon={value.faUIcon}
                ...
                <p
                  className={` ${montserrat.className}  w-[8em] h-full flex justify-center items-center`}
                >
                  {value.name}
                </p>
              </button>
      ...
      <li
        className="w-full h-[3em] text-[#fff] flex justify-center items-center
                hover:bg-[#fff] hover:text-[#151515] rounded-[.5em]"
      >
        <button
          className=" w-[90%] h-full flex justify-center px-0.5 items-center
                flex-row rounded-[.5em] active:bg-[#fff] active:text-[#000]
                focus:bg-[#fff] focus:text-[#151515]
                "
        >
