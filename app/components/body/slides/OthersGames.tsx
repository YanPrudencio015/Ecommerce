...
export default function OthersGames({ windowSize }: MostPopularprops) {
  ...
  useEffect(() => {
    ...
    let adventure = {
      url: "api/games?type=adventure",
      data: setAdventureGames,
    };
  ...
  useLayoutEffect(() => {
    ...
    const allCategoriesLoaded = gamesCategories.every(
    ...
    const allGamesValid = gamesCategories.every((category) =>
      category.every(
        (game) => game && game.cover && game.cover.url && game.name,
      ),
    );
  ...
  return (
    <div
      className="w-full h-[50em] sm:h-[40em] lg:h-[50em] flex justify-center 
    ...
    >
      <Swiper
        ...
        spaceBetween={5}
        loop={Racingames.length > calculate(windowSize)}
      ...
      >
        ...
        {gamesCategories.map((value, index) => (
          <SwiperSlide
            key={index}
            className={` flex! justify-between items-center flex-col gap-3.5
                    py-[2em]`}
          >
            {/* Slider title */}
            <h1 className={`${bebasNeue.className} text-[2em]`}>
              {value[0]?.genres?.[0]?.name}
            </h1>
            {/* games cards*/}
            {value.map((value, index) => (
              <div
                key={index}
                className=" w-[95%] h-auto rounded-2xl !flex justify-center 
                            items-center flex-col gap-3.5 sm:rounded-0"
              >
                {/* games cards background Border*/}
                <div
                  className=" w-full h-[6em] sm:h-[5em] lg:h-[6.5em] rounded-[.5em] flex justify-center items-center bg-gradient-to-bl
                    ...
                    <div className="w-[90%] h-full p-[1em] relative sm:w-[70%]">
                      <p
                        className={`${montserrat.className}
                                        w-full text-[.9em] sm:text-[.7em]`}
                      >
                        {value.name}
                      </p>
                      <div
                        className=" w-[5em] flex justify-between flex-row absolute right-3 bottom-1
                                        lg:w-[3.5em] lg:h-[2em] items-center"
                      >
                        <button
                          className=" w-[2.5em] h-[2.5em] flex justify-center items-center rounded-full
                                        active:bg-[#BF092F] lg:w-[3em] lg:h-[2em] group"
                        >
                          <HeartIcon
                            className="size-7 text-[#BF092F] 
                        ...
                        >
                          <ShoppingBagIcon
                            className="size-7 text-[#068FFF] hover:text-[#fff] 
                                            group-active:text-[#fff] cursor-pointer lg:size-5"
                          />
                        </button>
                      </div>
                      <p
                        className={`${OpenSans.className} bg-[#068FFF] 
                                    w-[5em] flex justify-center 
                                    rounded-[.5em] lg:text-[.8em]`}
                      >
                        R$ 59.99
                      </p>
                    </div>
            ...
            <button
              className={`${lato.className} h-[1.5em] bg-[#fff] 
